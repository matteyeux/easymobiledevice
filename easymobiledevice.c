...
int lire_clavier (char *cmd, int taille)
{	
  	int i=0;
...
{	
	...
	printf("Commande : ");
	scanf ("%d", &command);
	lire_clavier(cmd, sizeof(cmd));
	
	//printf("Entrez votre UDID : n");
	//scanf("%s", UDID);



	if (command == 23 || !strcmp(cmd, "ideviceactivation"))
	{
		printf("OK\n");
	}
	...
	{	
		int recovery;
		printf("1) Envoyer un fichier vers l'appareil\n");
		printf("2) Envoyer un payload grâce à limera1n\n");
		printf("3) Sortir l'appareil du Recovery Mode\n");
		printf("4) Executer un script recovery\n");
		scanf("%d", &recovery);
		printf("%d\n", recovery);

		if (recovery == 1)
		{	
			printf("Selectionner le repertoire vers le fichier à envoyer : ");
